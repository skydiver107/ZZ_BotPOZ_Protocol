            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"4a697f690d1f46e0f1a897a8662acae31ced3039b00c052392ed0bc179f9f28c" // init code hash
                    )
                )
            )
        );
...
interface INATIVE {
    function dynamicAdjustment(bool tradeType) external;
}
...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    // function _nativeAdjust(address[] memory path) internal {
    //     if (path[0] == native || path[path.length - 1] == native) {
    //         bool tradeType = path[path.length - 1] == native;
    //         INATIVE(native).dynamicAdjustment(tradeType);
    //     }
    // }
